...
def get_latest_tickers():
    ...
    try:
        ...
        st.warning(f"Could not fetch the latest ticker list: {e}. Using a default list.")
        return pd.DataFrame({
    ...
    if info is None:
            ...
            if analysis_key not in st.session_state.ai_analysis:
                if st.button("AI 최종 의견 생성하기", key="verdict_button", use_container_width=True):
                    with st.spinner("AI가 모든 데이터를 종합하여 최종 투자 의견을 생성 중입니다..."):
                        ...
                        st.session_state.ai_analysis[analysis_key] = generate_ai_analysis(info, (scores, details), 'verdict_stock')
            
            if analysis_key in st.session_state.ai_analysis:
                st.markdown(st.session_state.ai_analysis[analysis_key])
            else:
                st.info("버튼을 클릭하면 제미나이 AI가 모든 데이터를 종합하여 최종 투자 의견을 제시합니다.")

        tab1, tab2, tab3 = st.tabs(["**📊 대시보드 및 차트**", "**📂 재무 및 가치평가**", "**💡 뉴스 분석**"])
        ...
        with tab1:
            ...
            if not history.empty:
                ...
                with st.expander("🤖 AI 심층 차트 분석 보기"):
                    analysis_key = 'chart'
                    if analysis_key not in st.session_state.ai_analysis:
                        if st.button("AI 차트 분석 실행", key="chart_run"):
            ...
            with st.container(border=True):
                ...
                if financials is not None and not financials.empty:
                    ...
                    with st.expander("🤖 AI 재무 진단 보기"):
                        ...
                        if analysis_key not in st.session_state.ai_analysis:
                            if st.button("AI 재무 진단 실행", key="financial_run"):
        ...
        with tab3:
            with st.container(border=True):
                ...
                analysis_key = 'news'
                if analysis_key not in st.session_state.ai_analysis:
                     if st.button("AI 뉴스 분석 실행", key="news_run"):
